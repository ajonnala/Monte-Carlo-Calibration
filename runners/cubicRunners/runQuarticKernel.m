...
s = strcat('~/private/Monte-Carlo-Calibration/Data/Cubic_Spline_Quartic_TH:',int2str(timeHorizon),'strikes:',int2str(numStrikes),'timesteps:',int2str(timeSteps),'numParticles:',int2str(numParticles),'.txt');
file = fopen(s,'a');
%for initialPrice = 0.2:0.2:4
       tStart = cputime;
        [expectation, strikes] = particleCubicQuartic(initialPrice, timeSteps, timeHorizon, ...
